...
func TestAppConfigReadWriteUsesAllowlist(t *testing.T) {
	...
	service := &AppConfigService{allowed: map[string]map[string]bool{"podman": {path: true}}}

	initial, err := service.Read("podman", path)
	if err != nil || initial.Exists || initial.Content != "{}\n" {
		t.Fatalf("initial config = %#v, %v", initial, err)
	}
	if _, err := service.Write("podman", path, "{\"debug\":true}\n"); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := service.Read("caddy", path); !errors.Is(err, ErrAppConfigDenied) {
...
func TestAppConfigRejectsInvalidJSON(t *testing.T) {
	...
	service := &AppConfigService{allowed: map[string]map[string]bool{"podman": {path: true}}}
	if _, err := service.Write("podman", path, "not json"); !errors.Is(err, ErrAppConfigInvalid) {
